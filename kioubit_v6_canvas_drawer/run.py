...
hosts_file = sys.argv[1]  # 存储IP地址的JSON文件路径
...
async def sping(ip):
    print(f"Ping {ip}")
    #icmpv6_bare_request(ip, 'fd42:1877:222c::2')
    #subprocess.Popen(["ping", "-w", "0", "-c", "1", ip],stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    subprocess.Popen(["fping", "-6", "-q", "-t", "1", ip],stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    

async def ping_all(ips):
    tasks = []
    
    #aip_list = ['fdcf:8538:9ad5:3333:' + ip for ip in ips if ip != '']
    #aip = ' '.join(aip_list)
    
    #print(f"Ping {aip}")
    #subprocess.Popen(["fping", "-6", "-q", "-t", "1", aip],stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    for ip in ips:
...
if __name__ == '__main__':
    ...
    while True:
        for ip in ips:
            print(f"Ping fdcf:8538:9ad5:3333:{ip}")
            icmpv6_bare_request(f'fdcf:8538:9ad5:3333:' + ip, 'fd42:1877:222c::2')
